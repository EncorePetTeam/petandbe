...
class FilteringControllerTest {
    ...
    void filteringAccommodationSuccess() throws Exception {
        ...
        for(int i=0;i<10;i++){
            ...
            FilteringAccommodationResponse filteringAccommodationResponse = new FilteringAccommodationResponse(id,accommodationName,address,rate);
            filteringAccommodationResponses.add(filteringAccommodationResponse);
        }

        FilteringAccommodationListResponse filteringAccommodationListResponse = new FilteringAccommodationListResponse(filteringAccommodationResponses);

        when(filteringService.filteringAccommodation(any(FilteringAccommodationRequests.class))).thenReturn(filteringAccommodationListResponse);
        //when
        ResultActions resultActions = mockMvc.perform(RestDocumentationRequestBuilders
                .get("/filtering/filtering-accommodation")
                .params(info)
        ...
        resultActions.andExpect(status().isOk())
                .andDo(document("filtering-accommodation",
                        ...
                        responseFields(
                                ...
                                fieldWithPath("filteringAccommodationList[].accommodationId").type(JsonFieldType.NUMBER).description("숙소 Id"),
                                fieldWithPath("filteringAccommodationList[].accommodationName").type(JsonFieldType.STRING).description("숙소 이름"),
                                fieldWithPath("filteringAccommodationList[].address").type(JsonFieldType.STRING).description("숙소 주소"),
                                fieldWithPath("filteringAccommodationList[].avgRate").type(JsonFieldType.NUMBER).description("숙소 평점")
                        )
                )).andDo(print());
    }
